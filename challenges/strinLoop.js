...
print its even-indexed and odd-indexed characters as 2space-separated strings on a single line 
(see the Samples below for more detail).
...
function processData(input) {
    ...
    // We need to get a strings array from the input,
    //  without taking the first string which is the number of inputs T
    let strArray = input.split("\n");
    let T = strArray.shift();

    // strArr will now contain T number of string elements
    // cycle through each string in array to get odd and even characters
    strArray.forEach((strj) => {
        let evenStr = '';
        let oddStr = '';
        for(let i = 0; i < strj.length; i++) {
            i % 2 ? oddStr += strj[i] : evenStr += strj[i];
        }
